...
LARGE_FILES = {
    ...
    'bert_similarity.pkl':    '1gl_M0YeW5WLr5jZcPhGP2S7sMBvCLO0K',
    'als_model.pkl':          '1w5EJ7xVcv0GrsZ1y1cAWJAt_5iC__K0K',
...
def download_large_files():
    all_present = all(os.path.exists(f) for f in LARGE_FILES)
    if all_present:
        return
    st.info("⬇️ First run: downloading model files from Google Drive... (this takes 2-3 minutes)")
    progress = st.progress(0)
    for i, (filename, file_id) in enumerate(LARGE_FILES.items()):
        if not os.path.exists(filename):
            st.write(f"Downloading {filename}...")
            url = f"https://drive.google.com/uc?id={file_id}"
            gdown.download(url, filename, quiet=False)
        progress.progress((i + 1) / len(LARGE_FILES))
    st.success("✅ All files downloaded! Loading app...")
    st.rerun()
